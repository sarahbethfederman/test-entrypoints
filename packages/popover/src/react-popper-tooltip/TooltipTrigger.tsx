...
import { GetTriggerPropsArg, TooltipTriggerProps, TooltipTriggerState } from './types';
...
class TooltipTrigger extends React.Component<TooltipTriggerProps, TooltipTriggerState> {
    ...
    return (
      <Manager>
        <Reference innerRef={getTriggerRef}>
          {({ ref }) => children({ getTriggerProps: this.getTriggerProps, triggerRef: ref })}
        </Reference>
  ...
  private hideTooltip = () => {
    this.clearScheduled();
    this.hideTimeout = window.setTimeout(() => this.setTooltipState({ tooltipShown: false }), this.props.delayHide);
  };
