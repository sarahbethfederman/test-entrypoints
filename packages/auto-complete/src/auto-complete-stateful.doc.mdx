...
## AutoCompleteStateful Properties
## Properties
<PropTable>
  ...
  <PropTable.Entry name="backupOption" type="{label, value}"
   description="Optional - Display provided backupOption if no match found."/>
  ...
  <PropTable.Entry name="onSelectItem" type="(selectedItem: string | number)=> void" description="Selected item from the datasource. Also triggered with empty string as value when input field is cleared through pressing X button."/>
  ...
  <PropTable.Entry name="initialValue" type="string" description="initial value displayed in input element"/>
</PropTable>
