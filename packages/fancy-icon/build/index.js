...
const fancyIconTemplate = require('./fancy-icon.template');
const indexTemplate = require('./index.template');
const util = require('./util');

const SOURCE_PATH = path.join(__dirname, '../src/fancy-icons');
const DESTINATION_PATH = path.join(__dirname, '../src/fancy-icons-compiled');
const INDEX_PATH = path.join(__dirname, '../src');

function generateReactFromSvg(svgr, svgCode, componentName) {
  return svgr.default(svgCode, { fancyIcon: true }, { componentName: util.pascalCase(componentName) }).then((code) => ({
    code: fancyIconTemplate(code),
      ...
      resolvedFiles
        .map(({ code, fileName }) => {
          ...
          code = code
            ...
            .replace(/{\.\.\.props}/, '')
            .replace(/\(props\)/, '(props)')
            .replace(/<svg/, '<FancyIconWrapper {...props}>\n\t<svg')
